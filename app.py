...
def _export_vulnerabilities_pdf(scan):
    ...
    # Title
    content.append(Paragraph(f"Vulnerability Report - {scan.get('project', 'Unknown')}", title_style))
    content.append(Spacer(1, 0.2*inch))
    
    # Scan information
    scan_info = [
        ['Project', scan.get('project', 'Unknown')],
        ['Build Number', str(scan.get('build_number', 'N/A'))],
        ['Branch/Environment', scan.get('branch_name', 'Not Provided')],
        ['Scan Date', format_datetime_tz(scan.get('timestamp'), tz)],
        ['Total Vulnerabilities', str(len(scan.get('vulnerabilities', [])))],
    ]
    ...
    scan_table = Table(scan_info, colWidths=[2*inch, 3.5*inch])
    ...
    content.append(Paragraph("Vulnerability Details", styles['Heading2']))
    ...
    content.append(Spacer(1, 0.2*inch))
    content.append(Paragraph(f"Report generated: {format_datetime_tz(datetime.now(), tz)}", styles['Normal']))
...
def _export_vulnerabilities_csv(scan):
    ...
    writer.writerow(['Total', len(vulnerabilities)])
    writer.writerow([])
    
    # Write vulnerability details header
    writer.writerow([
        'CVE ID',
        'Severity',
